...
CREATE PROCEDURE [DTG].[sel_TermByColumnsForTextReview]
...
AS
	...
	IF @DataOwner = 0
	BEGIN
		SELECT @DataOwner = NULL
	END 
	IF @DataManager = 0
	BEGIN
		SELECT @DataManager = NULL
	END 
	...
	SELECT
		...
		T.UpdateUserName,
		T.HashTag,
		(SELECT TR.TermRecommendationId, TR.TermId, TR.Name, TR.NameEN, TR.Description, TR.DescriptionEN, TR.Status, TR.UserName, TR.SystemDate, TR.HostName, TR.HostIP, TR.UpdateUserName, TR.UpdateHostName, TR.UpdateSystemDate FROM [DTG].[TermRecommendation] TR WITH(NOLOCK) WHERE TR.TermId = T.TermId FOR XML PATH('TermRecommendation')) AS TermRecommendations,
		(SELECT TTAJ.TermTextApprovalJournalId, TTAJ.TermId, TTAJ.TextApprovalStatus, TTAJ.Description, TTAJ.Status, TTAJ.UserName, TTAJ.SystemDate, TTAJ.HostName, TTAJ.HostIP, TTAJ.UpdateUserName, TTAJ.UpdateHostName, TTAJ.UpdateSystemDate FROM [DTG].[TermTextApprovalJournal] TTAJ WITH(NOLOCK) WHERE TTAJ.TermId = T.TermId FOR XML PATH('TermTextApprovalJournal')) AS TermTextApprovalJournals
