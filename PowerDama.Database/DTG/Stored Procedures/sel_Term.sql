...
*		Last Modified By		: hanifi
*		Last Modification Date	: 02/05/2018
*
*/
CREATE PROCEDURE [DTG].[sel_Term]
...
SET NOCOUNT ON;
	...
	LEFT JOIN [BOA].[COR].[BOAUSER] U WITH (NOLOCK)
			ON T.UserName = U.usercode
	LEFT JOIN [BOA].[COR].[BOAUSER] U2 WITH (NOLOCK)
			ON T.UpdateUserName = U2.usercode
	LEFT JOIN [DTG].[TermRule] TR2 WITH (NOLOCK) 
			ON TR2.TermId = T.TermId AND TR2.TemplateType = 1
